...
import { Plus, Link as LinkIcon, Image as ImageIcon, LogOut, Eye, EyeOff, Trash2, FolderOpen, LayoutGrid, ShieldCheck, Send, Stamp, Download, Pencil } from "lucide-react";
    ...
    return (
      <div style={styles.loginWrap}>
        <div style={styles.loginCard}>
          <div style={styles.loginLogoBadge}><img src={LOGO_URI} alt="el galpón creativo" style={styles.loginLogo} /></div>
      ...
      <div style={styles.headerLeft}>
        <div style={styles.logoRow}>
          <div style={styles.logoBadge}>
            <img src={LOGO_URI} alt="el galpón creativo" style={styles.logoMark} />
          </div>
        </div>
      </div>
...
function BriefsView({ profile, briefs, onReload, onOpenBrief, showToast }) {
  const [showForm, setShowForm] = useState(() => !!localStorage.getItem("draft-brief"));
  const [editingId, setEditingId] = useState(() => localStorage.getItem("draft-brief-editingId") || null);
  const [form, setForm] = useState(() => {
    const saved = localStorage.getItem("draft-brief");
    return saved ? JSON.parse(saved) : { titulo: "", cliente: "", descripcion: "", fecha: "", linkBrief: "" };
  });
  const [busy, setBusy] = useState(false);
  ...
  useEffect(() => {
    if (!showForm) return;
    try {
      localStorage.setItem("draft-brief", JSON.stringify(form));
      setDraftSaved(true);
    } catch (err) {
      setDraftSaved(false);
  ...
  function clearDraft() {
    localStorage.removeItem("draft-brief");
    localStorage.removeItem("draft-brief-editingId");
  }
  ...
  function openEditForm(b) {
    setEditingId(b.id);
    localStorage.setItem("draft-brief-editingId", b.id);
    setForm({ titulo: b.titulo, cliente: b.cliente, descripcion: b.descripcion || "", fecha: b.fecha_limite || "", linkBrief: b.link_brief || "" });
    setShowForm(true);
  }
  ...
  async function submitBrief() {
    ...
    try {
      const payload = {
        ...
        fecha_limite: form.fecha || null, link_brief: form.linkBrief || null,
      };
      const { error } = editingId
        ? await supabase.from("briefs").update(payload).eq("id", editingId)
        : await supabase.from("briefs").insert({ ...payload, creado_por: profile.id });
      if (error) return showToast(error.message);
      clearDraft();
      setForm({ titulo: "", cliente: "", descripcion: "", fecha: "", linkBrief: "" });
      setShowForm(false);
      setEditingId(null);
      showToast(editingId ? "Brief actualizado" : "Brief publicado");
      onReload();
    } catch (err) {
  ...
  return (
    <div>
      <div style={styles.sectionHead}>
        ...
        {canCreate && <button style={styles.primaryBtn} onClick={() => (showForm ? setShowForm(false) : openNewForm())}><Plus size={16} /> Nuevo brief</button>}
      </div>

      {showForm && (
        <div style={styles.folderForm}>
          <div style={styles.ideaFormTitle}>{editingId ? "Editando brief" : "Nuevo brief"}</div>
          ...
          <button type="button" disabled={busy} onClick={submitBrief} style={{ ...styles.primaryBtn, marginTop: 4 }}>{editingId ? "Guardar cambios" : "Publicar brief"}</button>
          {draftSaved && <span style={styles.draftNote}>✓ Borrador guardado automáticamente</span>}
        </div>
      )}
            ...
            <div key={b.id} style={styles.folderCard} onClick={() => onOpenBrief(b.id)}>
              ...
              {canCreate && (
                <div style={styles.folderActions} onClick={(e) => e.stopPropagation()}>
                  <button style={styles.iconBtn} onClick={() => openEditForm(b)} title="Editar"><Pencil size={13} /></button>
                  <button style={styles.iconBtn} onClick={() => deleteBrief(b.id)} title="Eliminar"><Trash2 size={13} /></button>
                </div>
              )}
...
function BoardView({ profile, briefs, ideas, onReload, selectedBriefId, setSelectedBriefId, showToast }) {
  const [form, setForm] = useState(() => {
    const saved = localStorage.getItem("draft-idea");
    return saved ? JSON.parse(saved) : { briefId: selectedBriefId || "", mostrarNombre: false, ideaTexto: "", link: "", imagen: "" };
  });
  const [busy, setBusy] = useState(false);
  ...
  useEffect(() => {
    const hasContent = form.ideaTexto || form.link || form.imagen;
    if (hasContent) localStorage.setItem("draft-idea", JSON.stringify(form));
    else localStorage.removeItem("draft-idea");
  }, [form]);
...
const styles = {
  ...
  loginLogo: { width: "100%", maxWidth: 280, height: "auto", display: "block" },
  loginLogoBadge: { background: "#14173B", borderRadius: 14, padding: "20px 24px", marginBottom: 14, display: "inline-flex" },
  ...
  logoBadge: { background: "#14173B", borderRadius: 10, padding: "8px 14px", display: "flex", alignItems: "center" },
  logoMark: { height: 40, width: "auto", display: "block" },
  logo: { fontFamily: "'Fraunces', serif", fontSize: 27, fontWeight: 600 },
  ...
  folderDesc: { fontSize: 12.5, color: "#6B6553", lineHeight: 1.5, minHeight: 34, whiteSpace: "pre-line" },
  ...
  folderActions: { position: "absolute", top: 10, right: 10, display: "flex", gap: 6 },
  iconBtn: { background: "#fff", border: `1px solid ${line}`, borderRadius: 3, padding: "5px 6px", cursor: "pointer", color: "#6B6553", display: "flex" },
  ...
  draftNote: { display: "inline-block", marginLeft: 12, fontSize: 11.5, color: "#5B8C6B", fontFamily: "'IBM Plex Mono', monospace" },
  ...
  ideaCardText: { fontSize: 14, lineHeight: 1.5, margin: "0 0 10px", whiteSpace: "pre-line" },
