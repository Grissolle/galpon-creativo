...
function AuthGate({ session, onProfileReady, showToast }) {
  ...
  async function handleAuth() {
    if (!email.trim() || !password.trim()) return showToast("Falta correo o contraseña");
    setBusy(true);
    if (mode === "login") {
...
function Header({ profile, view, setView, ideas, briefs, showToast }) {
  ...
  if (profile.rol === "director") tabs.push({ id: "director", label: "Panel del director", icon: ShieldCheck });
